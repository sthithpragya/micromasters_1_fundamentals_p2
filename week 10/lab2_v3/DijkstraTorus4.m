function route = DijkstraTorus4(input_map, start_coords, dest_coords, drawMap)
...
while true
    ...
    if j == 1 
        if map(i,ncols) == 1
           map(i,ncols) = 4;
           parent(i,ncols) = current;
           distances(i,ncols) = min_dist + 1;
        elseif map(i,ncols) == 4        
            if min_dist + 1 < distances(i,ncols)
               distances(i,ncols) = min_dist + 1;
               parent(i,ncols) = current;
            end
        elseif map(i,ncols) == 6
            parent(i,ncols) = current;
            distances(i,ncols) = min_dist + 1;
        end
    end
    ...
    if i == 1
        if map(nrows,j) == 1
           map(nrows,j) = 4;
           parent(nrows,j) = current;
           distances(nrows,j) = min_dist + 1;
        elseif map(nrows,j) == 4        
            if min_dist + 1 < distances(nrows,j)
               distances(nrows,j) = min_dist + 1;
               parent(nrows,j) = current;
            end
        elseif map(nrows,j) == 6
            parent(nrows,j) = current;
            distances(nrows,j) = min_dist + 1;
        end
    end
