function route = DijkstraTorus3(input_map, start_coords, dest_coords, drawMap)
...
dest_node2 = sub2ind(size(map), dx1,  dy2); 
dest_node3 = sub2ind(size(map), dx2,  dy1); 
dest_node4 = sub2ind(size(map), dx2,  dy2); 
...
while true
    ...
    if current == dest_node || current == dest_node2 || current == dest_node3 || current == dest_node3 || isinf(min_dist) 
        break
    end
    ...
    if j == ncols
        map(:,1) = map(:,ncols);
        distances(:,1) = distances(:,ncols);
        parent(:,1) = parent(:,ncols); 
        if map(i,2) == 1
           map(i,2) = 4;
           parent(i,2) = current;
    ...
    if j == 1
        map(:,ncols) = map(:,1);
        distances(:,ncols) = distances(:,1);
        parent(:,ncols) = parent(:,1); 
        if map(i,ncols-1) == 1
           map(i,ncols-1) = 4;
    ...
    if i == nrows
        map(1,:) = map(nrows,:);
        distances(1,:) = distances(nrows,:);
        parent(1,:) = parent(nrows,:);
        if map(2,j) == 1
           map(2,j) = 4;
           parent(2,j) = current;
    ...
    if i == 1
        map(nrows,:) = map(1,:);
        distances(nrows,:) = distances(1,:);
        parent(nrows,:) = parent(1,:);
        if map(nrows-1,j) == 1
           map(nrows-1,j) = 4;
