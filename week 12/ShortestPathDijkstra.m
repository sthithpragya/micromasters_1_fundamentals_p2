...
%     distances(idx) = Inf;
%     if isempty(i) ~= 1
%         num_of_adjacent_nodes = size(i,2);
%         for j = 1:num_of_adjacent_nodes
%             if distances(i(j)) == Inf || distances(i(j)) > min_dist + map(idx, i(j))
%                 distances(i(j)) = min_dist + map(idx, i(j));
%                 map(i(j),idx) = 0;
%                 parent(i(j)) = idx;
%             end
%         end
%     end
    ...
    if isempty(i) ~= 1
        num_of_adjacent_nodes = size(i,2);
        for j = 1:num_of_adjacent_nodes
            if distances(i(j)) == Inf && isnan(parent(i(j))) == 0
            else
                if distances(i(j)) == Inf || distances(i(j)) > min_dist + map(idx, i(j))
                    distances(i(j)) = min_dist + map(idx, i(j));
                    parent(i(j)) = idx;
